...
model=tf.keras.models.load_model('../2.target_scan/model_smRNA/')

split_dataset('cropped/',DATASET_PATH='dataset',train_images = 1, val_images = 0,test_images = 0)
samp=train_generator.image_groups
pr=model.predict_classes(train_generator)
res=pd.DataFrame({'Sample':samp[0],'Class':pr.reshape(552)})
res[['drop','srna','mrna','drop']]=res['Sample'].str.extract('(.*)/(cand.*)-(.*).(csv)$')
res=res[['srna','mrna','Class']]
res_filtered=res.loc[res['Class']==1]
res_filtered.to_csv('filtered_couples.csv',index=False)